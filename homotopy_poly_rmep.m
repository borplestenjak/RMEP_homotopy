...
while run<maxruns && length(find(converged==1))<neig
    ...
    parfor ind = 1:neig
        if converged(ind)==0
            ...
            Z0 = zeros(m,n);
            Z1a = zeros(n_cellA,kpar+1);
            Z1b = zeros(n_cellB,kpar+1);
            ...
            while (t<1-1e-15) && (step<maxsteps) && (abort_path==0)
                ...
                Ma0(1:m,n+1:end) = (1-t)*eval_rmep_der(Cb,suppB_h,y(n+1:end),Z1b) + t*eval_rmep_der(Ca,suppA_h,y(n+1:end),Z1a);
                Ma0(m+1,1:n) = 2*y(1:n)';
                ...
                while noconv && (abort_path==0)
                    ...
                    % initial residual
                    W = (1-tp)*eval_rmep(B,suppB_h,yp(n+1:end),Z0) + tp*eval_rmep(A,suppA_h,yp(n+1:end),Z0);
                    ...
                    while (norm(rhs)>tol*norm(W)) && (innerstep<maxinnersteps)
                        ...
                        Ma(1:m,n+1:end) = (1-tp)*eval_rmep_der(Cb,suppB_h,yp(n+1:end),Z1b) + tp*eval_rmep_der(Ca,suppA_h,yp(n+1:end),Z1a);
                        Ma(m+1,1:n) = 2*yp(1:n)';
                    ...
                    if norm(rhs)<tol*(1+norm(W)) && angle_vec<maxangle && angle_eig<maxangle
                        ...
                        if h<(t*minstepsize + fail_stepsize)
                           if disp_level==2 
                                fprintf('Aborted path  %5d, step:%5d, 1-t:%7.2e, h:%7.2e, |lambda(0)|: %7.2e\n',...
